<template>
    <div class="constructor_scroll">
        <div class="container-block" :class="{container_large: !$store.state.menuDesktopShow}">
            <div class="tittle">
                <h1 v-cloak>{{textContent.titleMain}}</h1>
                <div v-cloak class="drop_search">
                    <!--<search></search>-->
                </div>
            </div>
            <div class="subtitle">
...
<script>
    ...
    export default {
        ...
        created() {
            this.resetModule()
            this.setUrl(this.$store.state.apiUrls.contacts)
            this.setTextContent({
                titleMain: 'Контакты',
                addItemText: 'Добавить',
                notFoundText: 'Контакты не найдено',
                addGreeting: 'Контакт успешно создан',
                deleteGreeting: 'Контакт успешно удален',
                titleAdd: 'Добавить',
                cancelButton: 'Отмена',
